...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < horizontalImage.length; i++) {
			for (int j = 0; j < horizontalImage[0].length; j++) {
				horizontalImage[i][j] = image[i][horizontalImage[0].length - 1 - j];
			}
		}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		int k = 0;
		for (int i = verticalImage.length - 1; i >= 0; i--) {
			int l = 0;
			for (int j = 0; j < image[0].length; j++) {
				verticalImage[k][l] = image[i][j];
				l++;
			}
			k++;
		}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		Color[][] scaledColor = new Color[height][width];
		for (int i = 0; i< scaledColor.length; i++) {
			for (int j = 0; j< scaledColor[0].length; j++) {
				scaledColor[i][j] = image[(int)(i * ((double) image.length / height))][(int)(j * ((double) image[0].length) / width)];
			}
		}
		return scaledColor;
	}
